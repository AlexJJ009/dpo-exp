...
# Training hyperparameters (same regime as 4B, adjusted batch for memory)
BETA = 0.1
LEARNING_RATE = 5e-7
NUM_EPOCHS = 1
PER_DEVICE_BATCH_SIZE = 1  # reduced from 2 for 8B model memory
GRADIENT_ACCUMULATION_STEPS = 16  # effective batch = 16 (same as 4B)
MAX_LENGTH = 2048
WARMUP_RATIO = 0.1
WEIGHT_DECAY = 0.01
LR_SCHEDULER = "cosine"
...
def main():
    ...
    training_args = DPOConfig(
        ...
        seed=42,
        precompute_ref_log_probs=True,  # precompute ref logprobs to free ref model memory
        optim="adafactor",  # memory-efficient optimizer (factored second moments)
    )
