# Pipeline 监控提示词 — Qwen2.5-3B (base)
...
```bash
# 新开 tmux 窗口
tmux new-session -s monitor-qwen25-3b
...
tmux session `qwen25-3b-base` 中正在运行一个 Qwen2.5-3B DPO 全流程 pipeline（DeepSpeed ZeRO 2, 8 GPUs），包含 4 个顺序步骤：
...
```bash
# tmux session 是否存在
tmux has-session -t qwen25-3b-base 2>/dev/null && echo "SESSION: alive" || echo "SESSION: DEAD"
...
### 2. 读取日志末尾
```bash
tail -30 /data-1/dpo-experiment/run_qwen25_3b_base_pipeline.log
```
...
tail -30 /data-1/dpo-experiment/run_qwen25_3b_base_pipeline.log | grep -E "loss|margin|reward|NaN|Inf"
```
...
```bash
# 偏好对数量
wc -l /data-1/dataset/dpo/dpo-qwen25-3b-base/dpo-qwen25-3b-base-pairs.jsonl

# 训练 summary
python3 -c "
import json
with open('/data-1/checkpoints/qwen25-3b-base-dpo/training_logs/training_summary.json') as f:
    s = json.load(f)
r = s['results']
...
# 评测结果
cat /data-1/checkpoints/qwen25-3b-base-dpo/inference_n3/eval_metrics.json | python3 -m json.tool
```
...
```bash
# 检查各阶段产出是否存在
ls -lh /data-1/dataset/dpo/dpo-qwen25-3b-base/
ls -lh /data-1/checkpoints/qwen25-3b-base-dpo/ 2>/dev/null
ls -lh /data-1/checkpoints/qwen25-3b-base-dpo/inference_n3/ 2>/dev/null
```

重启脚本位置：`/data-1/dpo-experiment/run_qwen25_3b_base_pipeline.sh`
...
| 文件 | 用途 |
|------|------|
| `/data-1/dpo-experiment/run_qwen25_3b_base_pipeline.sh` | 主 pipeline 脚本 |
| `/data-1/dpo-experiment/run_qwen25_3b_base_pipeline.log` | 运行日志（主要检查对象） |
| `/data-1/dpo-experiment/dpo_pipeline/PIPELINE.md` | Pipeline 完整文档 |
| `/data-1/dpo-experiment/dpo_pipeline/train_dpo_qwen25_3b_base.py` | 训练脚本（DeepSpeed ZeRO 2） |
| `/data-1/dataset/dpo/dpo-qwen25-3b-base/` | 偏好对数据输出目录 |
| `/data-1/checkpoints/qwen25-3b-base-dpo/` | 模型 checkpoint 输出目录 |
| `/data-1/checkpoints/qwen25-3b-base-dpo/training_logs/training_summary.json` | 训练结果 summary |
| `/data-1/checkpoints/qwen25-3b-base-dpo/inference_n3/eval_metrics.json` | 评测结果 |
```
