#!/usr/bin/env bash
#
# One-click full pipeline for Qwen3-4B-Base DPO (v2 - strict filtering)
#
# Steps:
#   1. Generate preference pairs with --strict (think tags + answer completeness + verified incorrect)
#   2. Clean pairs (re-verify rejected responses with dual-layer verification)
#   3. Train DPO (beta=0.1, lr=5e-7, 1 epoch, effective batch=16)
#   4. Evaluate on 7 math benchmarks (n=3, temperature=1.0)
#
# Usage:
#   bash run_4b_full_pipeline.sh
#
# Requirements:
#   - Docker images: dpo-harness, verl-harness:latest
#   - 8x NVIDIA A800-SXM4-80GB GPUs available
#   - Source dataset at /data-1/dataset/EnsembleLLM-data/am_deepseek_r1_filtered_ad.jsonl
#   - Model cached at HF_HOME for Qwen/Qwen3-4B-Base
#
set -euo pipefail

# ==================== Configuration ====================
OUTPUT_PREFIX="dpo-4b-v2"
MODEL="Qwen/Qwen3-4B-Base"
CHECKPOINT_DIR="/data-1/checkpoints/qwen3-4b-base-dpo-v2"
...
echo "============================================================"
echo "  Qwen3-4B-Base DPO Full Pipeline (v2 - strict filtering)"
echo "============================================================"
echo ""
echo "  Model:            ${MODEL}"
echo "  Output prefix:    ${OUTPUT_PREFIX}"
echo "  Checkpoint:       ${CHECKPOINT_DIR}"
echo "  Strict filtering: YES (think tags + answer completeness + verified incorrect)"
...
echo "    batch_size=2, grad_accum=8 (effective=16)"
...
docker run --rm --gpus all --ipc=host \
  ...
  -w /data-1/dpo-experiment \
  dpo-harness \
  python dpo_pipeline/train_dpo_4b_v2.py
